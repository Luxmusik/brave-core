...
bool EthJsonRpcController::EnsProxyReaderResolveAddress(
  ...
  auto internal_callback = base::BindOnce(
      &EthJsonRpcController::OnEnsProxyReaderResolveAddress,
      base::Unretained(this), std::move(callback));
  std::string data;
  if (!ens::ResolveAddress(domain, &data)) {
    return false;
  }
...
void EthJsonRpcController::OnEnsProxyReaderResolveAddress(
  ...
  std::string ipfs = ipfs::ContentHashToIpfs(contenthash);
  DLOG(INFO) << "decoded ipfs:" << ipfs;
  if (callback)
    std::move(callback).Run(true, ipfs);
}
