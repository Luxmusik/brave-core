...
namespace {
const char kValidCatalog[] = "catalog.json";
const char kInvalidCatalog[] = "invalid_catalog.json";
}  // namespace
...
TEST_F(BatAdsEpsilonGreedyBanditResourceTest,
  ...
  const base::Optional<std::string> opt_value =
      ReadFileFromTestPathToString(kValidCatalog);
...
TEST_F(BatAdsEpsilonGreedyBanditResourceTest,
    FailToInitializeWithInvalidCatalog) {
  // Arrange
  const base::Optional<std::string> opt_value =
      ReadFileFromTestPathToString(kInvalidCatalog);
  ASSERT_TRUE(opt_value.has_value());
